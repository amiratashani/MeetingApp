...
import com.example.meetingappyt.netwoek.ApiClient;
import com.example.meetingappyt.netwoek.ApiService;
...
public class OutgoingInvitationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ImageView imageViewType = findViewById(R.id.imageMeetingType);
        ...
        if (meetingType != null) {
            if (meetingType.equals("video")) {
                ...
                imageViewType.setImageResource(R.drawable.ic_audio);
            }
        }
    ...
    private void initiateMeeting(String meetingType, String receiverToken, ArrayList<User> receivers) {
        try {
            ...
            date.put(Constants.REMOTE_MSG_TYPE, Constants.REMOTE_MSG_INVITATION);
            date.put(Constants.REMOTE_MSG_MEETING_TYPE, meetingType);
            date.put(Constants.KEY_FIRST_NAME, preferenceManager.getString(Constants.KEY_FIRST_NAME));
            date.put(Constants.KEY_LAST_NAME, preferenceManager.getString(Constants.KEY_LAST_NAME));
            date.put(Constants.KEY_EMAIL, preferenceManager.getString(Constants.KEY_EMAIL));
            date.put(Constants.REMOTE_MSG_INVITER_TOKEN, inviterToken);

            meetingRoom = preferenceManager.getString(Constants.KEY_USER_ID) + "_" +
                    UUID.randomUUID().toString().substring(0, 5);
            date.put(Constants.REMOTE_MSG_MEETING_ROOM, meetingRoom);

            JSONObject body = new JSONObject();
            body.put(Constants.REMOTE_MSG_DATA, date);
    ...
    private void cancelInvitation(String receiverToken, ArrayList<User> receivers) {
        try {
            ...
            JSONObject date = new JSONObject();
            date.put(Constants.REMOTE_MSG_TYPE, Constants.REMOTE_MSG_INVITATION_RESPONSE);
            date.put(Constants.REMOTE_MSG_INVITATION_RESPONSE, Constants.REMOTE_MSG_INVITATION_CANCELLED);
